        ...
        for this_frame, this_label in zip(frames, labels):
            ...
            if len(all_data) > 0 and len(all_data) == video_buffer_len:
                ...
                if Create_sam_feature == True:
                    ...
                    video_buff_GPU = np.transpose(this_video_buff,(1,0,2,3))# Reshape to (29, 3, 64, 64)
                    # input_resample =   F.interpolate(video_buff_GPU,  size=(128,  128), mode='bilinear', align_corners=False)
                    
                    bz,  ch, H, W = video_buff_GPU.shape
                    predicted_vdieo_masks =[]
                    with torch.no_grad():

                        for i in range(bz):
                            
                            input_img = video_buff_GPU[i:i+1] 
                            input_img = np.transpose(input_img,(0,2,3,1))[0,:,:,:]
                            masks = mask_generator.generate(input_img.astype((np.uint8)))

                            predicted_vdieo_masks.append(masks)
                            print("sam mask created:video"  + str(file_counter)  + " frame " + str(i) )
                    ...
                    sam_pkl_file_path = os.path.join(output_folder_sam_masks, sam_pkl_file_name)

                    with open(sam_pkl_file_path, 'wb') as file:
                        pickle.dump(predicted_vdieo_masks, file)
